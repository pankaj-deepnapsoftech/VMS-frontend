...
export const WorkItemValidation = object({
  ...
  Severity: string()
    .oneOf(["Low", "Medium", "High"], "Select a valid Severity")
  ...
  Status: string()
    .oneOf(["Open", "Closed", "Pending"], "Select a valid Status")
    .required("Status is required"),
  Remediate_Upcoming_Time_Line: string().required(
    "Upcoming Timeline is required"
  ),
});
...
export const AddVulnerableData = object({
  ...
  CVE_ID: string().when("CVE", {
    ...
    then: (schema) =>
      schema
        .matches(
          /^CVE-\d{4}-\d{4,}$/,
          "Invalid CVE ID format (e.g., CVE-2023-12345)"
        )
        .required("required"),
  ...
  Location: string().url("Invalid URL").required("Required"),
  Title: string()
    .required("Required")
