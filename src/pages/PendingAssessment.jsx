...
import { CreateScheduleAssessment, deleteAssesment, getPendingAssessment } from "@/services/assessment.service";
...
const PendingAssessment = () => {
  ...
  const { data: pendingAssessment, isLoading: isPendingAssessmentLoading } = useQuery({
    queryKey: ["pending-assessment", { page, tenant }],
    queryFn: () => getPendingAssessment({ page, tenant }),
    enabled: !!token,
    placeholderData: keepPreviousData
  })

  const { mutate: DeleteAssesment, isPending: isDeleteAssesmentLoading } = useMutation({
  ...
  <div className="w-full  pb-20 p-6">
    <div className="mb-8">
      ...
      <p className="text-slate-300">
        Configure and schedule your pending assessment parameters
      </p>
    </div>
    <div className="bg-[#1a1f2e] rounded-lg shadow-xl overflow-hidden">
      ...
      {/* Table */}
      {pendingAssessment?.length < 1 ? (
        <NoDataFound />
      ) : (
        <div className="overflow-x-auto custom-scrollbar w-full">
          {isPendingAssessmentLoading ? <AssessmentSkeleton /> : <table className="min-w-full text-sm text-left text-gray-300 divide-y divide-gray-700">
            ...
            <tbody className="divide-y divide-gray-700">
              {pendingAssessment.map((item, index) => (
                <tr
                  key={item._id}
                  ...
                  <td className="px-4 py-3 flex gap-2">
                    {isDeleteAccess() && (
                      <button
                        ...
                        onClick={() =>
                          window.confirm("Delete this assessment?") &&
      ...
      <Pagination
        page={page}
        setPage={setPage}
        hasNextPage={pendingAssessment?.length === 10}
        total={pendingAssessment?.length}
      />
    </div>
