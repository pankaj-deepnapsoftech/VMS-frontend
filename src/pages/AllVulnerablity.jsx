...
import { useAllVulnerabililtyContext } from '@/context';
import { useFormik } from 'formik';
import { WorkItemValidation } from '@/Validation/AllVulnerabililtyValidation';

export function AllVulnerability() {

  const { UpdateData } = useAllVulnerabililtyContext();


  const { allVulnerabilityData } = useAllVulnerabililtyContext();
          ...
          <thead className="bg-blue-100">
            <tr>
              {dynamicHeaders.map((header, index) => (
                <th
                  key={index}
                  className="px-4 py-3 text-left text-xs font-medium text-blue-800 uppercase tracking-wider cursor-pointer hover:bg-blue-200"
