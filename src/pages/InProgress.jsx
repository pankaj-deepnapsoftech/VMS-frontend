...
const PendingAssessment = () => {
  ...
  const { mutate: UpdateAssesment, isPending: isUpdateAssesmentLoading } =
    useMutation({
      ...
      onSuccess: async () => {
        await Promise.all([
          queryClient.invalidateQueries({ queryKey: ["pending-assessment"] }), // 🔥 IMPORTANT
          queryClient.invalidateQueries({
            queryKey: ["in-progress-assessment"],
          }),
          queryClient.invalidateQueries({ queryKey: ["completed-assessment"] }),
        ]);
      },
  ...
  const { data: progressAssessment, isLoading: isProgressAssessmentLoading } =
    useQuery({
