...
const RiskRatingTable = () => {
  ...
  const { data, RiskRatingDelete, page, setPage } = useContext(RiskRatingContext)
  ...
  return (
    <div className="p-4 h-full">
      <div className="flex justify-between">
        ...
        <button
          onClick={() => { setShowModal(!showModal); setEditTable(null) }}
          className="bg-sky-600 mb-2 text-white font-[500] py-2 px-5 rounded-lg shadow hover:bg-sky-700 focus:outline-none transition duration-300"
        >
          Add Risk Rating
        </button>
      </div>
        ...
        <table className="min-w-full bg-background ">
          <thead className="bg-background text-[15px]  text-white ">
            <tr>
              ...
              <th className="px-4 py-3 text-left whitespace-nowrap  truncate">PII</th>
              <th className="px-4 py-3 text-left whitespace-nowrap  truncate">Business Sensitive</th>
              <th className="px-4 py-3 text-left whitespace-nowrap  truncate">Regulations</th>
              <th className="px-4 py-3 text-left whitespace-nowrap  truncate">Security Confidentiality</th>
              <th className="px-4 py-3 text-left whitespace-nowrap  truncate">Security Integrity</th>
              <th className="px-4 py-3 text-left whitespace-nowrap  truncate">Security Availability</th>
              <th className="px-4 py-3 text-left whitespace-nowrap  truncate">Overall Risk Rating</th>
              ...
              >
                ...
                <td className="p-2 flex items-center justify-center relative top-2 gap-2">
                  <button
                    ...
                    type="button"
                    onClick={() => { setEditTable(item); setShowModal(true) }}
                  >
                    <FaEdit size={18} />
                  </button>
                  <button
      ...
      <RiskRating setShowModal={setShowModal} showModal={showModal} editTable={editTable} />
    </div>
  );
};
