...
import { FaBook, FaBug, FaChartBar, FaCheckCircle, FaClipboardList, FaClock, FaCode, FaCrosshairs, FaDragon, FaFileAlt, FaHeartbeat, FaKey, FaNetworkWired, FaSearch, FaShieldAlt, FaSpider, FaSyncAlt, FaTools } from "react-icons/fa";
...
function GettingStarted() {
	...
	const features = [
		{ icon: <FaSearch />, title: 'Resource discovery', description: 'First, the scanner attempts to discover various endpoints, sensitive files, and hidden paths on the target website.' },
		{ icon: <FaSpider />, title: 'Spidering', description: 'Based on the target URL and endpoints it discovered in the previous phase, the Website Scanner starts to map the structure of the web application.' },
		{ icon: <FaBug />, title: 'Active scanning', description: 'While the Spider is running, another component of the Website Scanner takes each previously discovered endpoint and tests for vulnerabilities.' },
		{ icon: <FaHeartbeat />, title: 'Passive scanning', description: 'To detect interesting information and leaked sensitive data, the Website Vulnerability Scanner also analyzes web traffic passively.' },
		{ icon: <FaShieldAlt />, title: 'Version-based CVE detection', description: 'Besides specific application vulnerability detection, the Website Vulnerability Scanner also discovers vulnerabilities based on software versions.' },
		{ icon: <FaClipboardList />, title: 'Full list of security tests performed', description: 'The deep version of the Website Vulnerability Scanner performs a complete security assessment of your web application.' }
	];
	...
	const newFeatures = [
		{
			...
			description: "A cloud-based scanner, the Website Vulnerability Scanner on Pentest-Tools.com does not require installation, configuration, or maintenance. Just launch a scan, enter your target URL, and enhance results with advanced scan setup."
		},
		{
			icon: <FaSyncAlt className="text-4xl text-blue-400" />,
			title: "Automate website security scans with flexible scheduling",
			description: "New vulnerabilities show up on the web all the time. Friday nights, Mondays, anytime. Stay ahead with scheduled scans: daily, weekly, or monthly. The moment our scanner learns of a new vulnerability, it starts looking for it automatically."
		},
		{
			icon: <FaCode className="text-4xl text-blue-400" />,
			title: "Integrate, automate, and streamline with our API",
			description: "Many security leaders use a few pentesting tools, not just our scanner. That’s why we integrate with CI/CD pipelines, security orchestration platforms, and development frameworks, making security testing an integral part of your development process."
		},
		{
			icon: <FaNetworkWired className="text-4xl text-blue-400" />,
			title: "Scan internal web apps without making assets public",
			description: "Need to scan assets behind firewalls, on private clouds, or internal networks? Our VPN feature enables this securely so you can detect risks without exposing your assets to the internet."
		},
		{
			icon: <FaChartBar className="text-4xl text-blue-400" />,
			title: "Integrate scan results into the tools you already use",
			description: "Vulnerabilities can slip through the cracks when they’re hard to monitor. Sync security alerts with Jira, GitHub, Slack, and other platforms so your team gets results instantly."
		}
	];

	return (
		<div className="min-h-screen bg-[#015289] p-8 flex justify-center flex-col items-center">

			<div className="container mx-auto flex flex-col lg:flex-row justify-start items-start lg:items-end  ">


				{/* Left Section */}
				<div className="max-w-2xl ml-5">
					<div className="bg-white w-20 h-20 rounded-xl flex items-center justify-center shadow-xl mb-6">
						<FaSpider className="w-12 h-12 text-[#01406b]" />
					</div>
				...
				<div className="bg-white backdrop-blur-lg rounded-2xl shadow-2xl p-12 max-w-md w-full text-center transform transition-all duration-300 hover:scale-105 lg:ml-32 mb-20 lg:mt-20 mt-20">
					<div className="text-center mb-5">
						<h1 className="text-2xl font-bold text-gray-800">Welcome to <span className="text-blue-600">SECURE&</span>
						</h1>
						<p className="text-xs text-gray-600 font-semibold">Beyond security, a Strategy</p>
					</div>
			...
			<ul className="mt-14 flex flex-wrap gap-x-9 gap-y-3 py-3 md:mx-auto md:w-fit">
				{links.map(({ href, icon, label, target }, index) => (
					<li key={index}>
						<a
							href={href}
							target={target}
							className="flex items-center gap-2 py-3 font-semibold text-white transition-colors hover:text-gray-500 hover:underline"
						>
							{icon} {label}
						</a>
					</li>
			...
			<div className="  text-white min-h-screen py-10 px-6 md:px-20">
				...
				<div className="relative flex flex-col items-center  px-5 bg-[#015289]   shadow-xl min-h-screen text-white">
					...
					{list.map((feature, index) => (
						<div
							key={index}
							className={`relative flex items-center w-full max-w-4xl mb-16 ${index % 2 === 0 ? "justify-start" : "justify-end"}`}
						>
							{/* Stepper Dot */}
							...
							<div className="w-5/6 md:w-1/2 bg-gray-100 shadow-xl border border-gray-100 rounded-2xl p-6 transition-transform hover:scale-105">
								<div className="flex items-center gap-5">
									<div className="p-3 bg-gray-300 rounded-full">{feature.icon}</div>
									<div>
										<h3 className="text-xl font-semibold text-black">{feature.title}</h3>
										<p className="text-gray-700 text-sm mt-2 leading-relaxed">{feature.description}</p>
									</div>
								</div>
							</div>
						</div>
			...
			<div className="min-h-screen bg-[#015289]   shadow-xl  m-10 text-white p-10 flex flex-col items-center rounded-lg">
				<div className="max-w-3xl text-left">
					<h2 className="text-3xl font-bold mb-4">How does the <span className="text-blue-500">Website Vulnerability Scanner</span> work?</h2>
					<p className="text-gray-100 text-lg">
						The Website Vulnerability Scanner is a DAST (Dynamic Application Security Testing) tool designed to discover
						vulnerabilities like XSS, SQL injection, and more. It interacts with the target application by sending HTTP
						requests with specific payloads to determine if a vulnerability exists.
					</p>
				</div>

				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-10 w-full max-w-6xl ">
					{features.map((feature, index) => (
						<div key={index} className="bg-gray-50  p-6 rounded-lg shadow-lg backdrop-blur-md border border-gray-700">
			...
			<div className="min-h-screenb bg-[#015289]   shadow-xl text-white flex flex-col md:flex-row items-center p-14 m-10 rounded-lg">
				<div className="md:w-1/3 pr-10">
					<h1 className="text-4xl font-bold mb-6 text-white">Easy enough for quick scans. <br /> Advanced enough for deep testing.</h1>
				</div>
				<div className="md:w-2/3 space-y-6">
					{newFeatures.map((newFeatures, index) => (
						<div key={index} className="flex items-center bg-white p-6 rounded-lg shadow-lg">
							...
							<div>
								<h2 className="text-xl font-semibold text-black">{newFeatures.title}</h2>
